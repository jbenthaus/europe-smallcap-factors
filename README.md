...
- **[Read the research note](paper/europe_smallcap_factors.pdf)** (PDF, 11 pages)
...
price-only returns, net of transaction costs (30 bps round-trip x turnover):

| Strategy | Period | Excess (ann.) | NW *t* | Sharpe | IR | Turnover |
|---|---|---:|---:|---:|---:|---:|
| Momentum + gross profitability (MP) | OOS 2020-25 | +8.9% | 4.04 | 0.71 | 1.31 | 261% |
| Momentum + quarterly profitability (MI) | OOS 2020-25 | +14.4% | 4.24 | 0.99 | 1.90 | 314% |
...
1.4 to 1.7 (MI; roughly 1.0 adjusted for return autocorrelation) with CAPM
market beta statistically indistinguishable from zero. A capacity study replays the strategy under participation-limited
...
![Out-of-sample excess returns by composite decile](paper/oos_decile_spread.png)
...
Returns are price-only (dividends excluded on both the strategy and
benchmark sides), and performance figures are net of
an assumed transaction cost (30 bps round-trip, with break-even cost levels
reported in the note) while attribution regressions stay gross, labeled.
Sharpe ratios use a zero risk-free rate and annualize under an i.i.d.
assumption; the note quantifies the autocorrelation correction this omits
and the sampling error of the ratios (standard error roughly 0.4 out of
sample).
Ratio levels therefore compare within these documents rather than across
publications.
...
author's; changes to core pipeline modules go through a per-change review
