...
with monthly rebalancing, earns an annualized excess return of 8.9\% over the
equal-weighted universe out of sample, net of assumed trading costs
(2020--2025; Newey--West $t = 4.04$,
information ratio 1.31), against 9.0\% ($t = 5.34$) in sample. A variant
replacing annual gross profitability with quarterly income-before-extraordinary-items
over assets earns 14.4\% ($t = 4.24$) out of sample. A market-neutral
...
1.4--1.7 for the quarterly variant (roughly 1.0 adjusted for return
autocorrelation), with CAPM market beta statistically
...
\paragraph{Returns.} Monthly returns are computed from the last available
daily close of each month, at the level of the individual security line,
with a data-error filter that removes
...
\section{Strategy construction}
\label{sec:factors}

Two strategies are evaluated, both equal-weighted composites of
two cross-sectional percentile ranks:

\begin{center}
\begin{tikzpicture}[
    sleeve/.style={draw, rounded corners, align=center, inner sep=5pt,
                   font=\small},
    comp/.style={draw, semithick, rounded corners, align=center,
                 inner sep=6pt, font=\small},
    w/.style={font=\scriptsize, midway, fill=white, inner sep=1.5pt},
  ]
  \node[sleeve] (mom) {12--2 momentum rank\\
    \scriptsize \citep{jegadeesh1993,carhart1997}};
  \node[comp] (mp) [below left=9mm and 8mm of mom]
    {\textbf{MP}\\ \scriptsize research reference};
  \node[comp] (mi) [below right=9mm and 8mm of mom]
    {\textbf{MI}\\ \scriptsize implementation variant};
  \node[sleeve] (gp) [below=9mm of mp]
    {gross profitability rank\\
     $(\mathrm{REVT}-\mathrm{COGS})/\mathrm{AT}$\\
     \scriptsize annual filings \citep{novymarx2013}};
  \node[sleeve] (roa) [below=9mm of mi]
    {quarterly ROA rank\\
     $\mathrm{IBQ}/\mathrm{ATQ}$\\
     \scriptsize quarterly filings \citep{hou2015}};
  \draw (mom) -- node[w] {$1/2$} (mp);
  \draw (mom) -- node[w] {$1/2$} (mi);
  \draw (gp)  -- node[w] {$1/2$} (mp);
  \draw (roa) -- node[w] {$1/2$} (mi);
\end{tikzpicture}
\end{center}

The two composites share the momentum sleeve and differ in a single
slot. MI's quarterly measure is a ROA-family profitability signal in the
spirit of the $q$-factor literature; its rationale is signal freshness,
four filing updates a year against one. The choice between MP and MI
was, however, made with out-of-sample results visible
(Section~\ref{sec:limitations}).
...
\paragraph{Timing discipline.} The same rules apply to every factor.
Accounting data enters factor ranks only from
...
evaluation and never moved. Transaction costs are charged at 30\,bps
round-trip against realized one-sided turnover, and performance figures
are net of modeled costs throughout. Attribution regressions
(Fama--MacBeth, factor-model alphas) are gross and labeled where they
appear: a cross-sectional regression has no portfolio cost concept, and
netting a time-series regression would make its estimates depend on the
cost assumption. Section~\ref{sec:results} reports the
resulting cost drag and the break-even cost levels. Significance of mean
monthly excess returns uses Newey--West standard
errors with 12 lags. Sharpe ratios are computed with a zero risk-free rate
throughout. For long-only series, deducting the euro short-term rate (ESTR,
averaging 1.3\% over this out-of-sample window) would lower the ratio by
roughly $r_f/\sigma$, about 0.08. Excess returns, their
$t$-statistics, and information ratios are unaffected. Sharpe and
information ratios annualize with $\sqrt{12}$ under an i.i.d.\ assumption.
Section~\ref{sec:limitations} quantifies the autocorrelation correction
this omits and the sampling error of the ratios.
...
\toprule
Strategy & Period & D10 & Bench & Excess & $t$ & Sharpe & IR & Max DD & Turnover \\
\midrule
MP & IS  (2007--19) & +13.4\% & +3.5\% & +9.0\%  & 5.34 & 0.92 & 1.37 & $-53\%$ & 265\% \\
MP & OOS (2020--25) & +12.2\% & +2.6\% & +8.9\%  & 4.04 & 0.71 & 1.31 & $-26\%$ & 261\% \\
MI & IS  (2007--19) & +17.3\% & +3.5\% & +12.5\% & 6.19 & 1.10 & 1.78 & $-54\%$ & 314\% \\
MI & OOS (2020--25) & +18.5\% & +2.6\% & +14.4\% & 4.24 & 0.99 & 1.90 & $-27\%$ & 314\% \\
\bottomrule
\end{tabular}
\caption{D10 long-only performance, EUR, price-only returns, net of
transaction costs at 30\,bps round-trip $\times$ one-sided turnover; the
benchmark is a costless paper index.
D10 and Bench are annualized compound returns of the portfolio and the
equal-weighted factor-eligible benchmark. Excess is the annualized mean
monthly return of D10 over Bench; as an arithmetic mean it does not equal
the difference of the compound columns. $t$ uses Newey--West standard
errors (12 lags); IR is the
...
(Section~\ref{sec:backtest}). The cost deduction amounts to 0.78\% (MP)
and 0.94\% (MI) per year; the break-even round-trip cost that zeroes
out-of-sample excess is $\approx$370\,bps (MP) and
$\approx$490\,bps (MI).}
\label{tab:headline}
\end{table}

Out-of-sample decay relative to in-sample is minimal for MP (9.0\% $\to$
8.9\%) and absent for MI, though Section~\ref{sec:limitations} qualifies
how much weight the out-of-sample figures can bear. The decile ladder
(Figure~\ref{fig:deciles}) is approximately monotone with a strongly
negative bottom decile, consistent with a genuine cross-sectional signal.

\paragraph{Factor-model alphas.} MP D10 long-only returns, gross of
trading costs (Section~\ref{sec:backtest}) and converted to USD
to match the factor basis, are regressed out of sample on the European
Fama--French factors \citep{fama2015} plus momentum, both from the
Kenneth R. French data library, with Newey--West
...
over passive factor exposure is decile concentration: D10 holds the top
10\% of small caps, while the factor portfolios are built from the top
30\% of all stocks.
...
\caption{Average monthly gross return by composite decile in excess of
the equal-weighted universe, MP, out-of-sample (71 formation months), a
signal diagnostic. Error bars are $\pm 1$ standard error of the monthly
mean.}
\label{fig:deciles}
\end{figure}
...
standard excess-return definition. Sharpe ratios annualize with
$\sqrt{12}$; the autocorrelation correction this omits is larger for the
hedged series than for the long-only series (Section~\ref{sec:limitations}).
Regressions use
...
The signal decays slowly in the holding horizon: pre-cost, a quarterly
rebalance retains roughly 87\% of the monthly alpha, and a semi-annual one
about 70\%. The effective lever at scale is therefore the execution budget
rather than the rebalance label. The binding constraint is structural
rather than per-name illiquidity: alpha concentrates at the smaller end of
the 180--1{,}800M band, so capacity-driven reallocation pushes the book
toward larger names where the factor edge is weakest.
...
Table~\ref{tab:capacity} reports the mode's net excess by AUM tier. Under
this execution model, the useful ceiling is near EUR 500M. A quarterly alternative, a
...
simulation's execution conventions differ from the backtest of
...
\begin{enumerate}
  ...
  headline $t$-statistics ($\geq$4.0) survive substantial haircuts. The
  ...
  ($+5.0\%$, $t{=}1.70$, gross, evaluated at an earlier pipeline vintage).
  ...
  market impact negligible at personal position sizes. Turnover counts name
  replacement on the equal-weight book; the smaller rebalancing trades that
  restore equal weights within continuing positions are not charged.
  ...
  with $\sqrt{12}$ under an i.i.d.\ assumption, the standard reporting
  convention, and no autocorrelation correction \citep{lo2002} is applied
  to the ratios. The correction would be material:
  Table~\ref{tab:sharpe_uncertainty} reports the adjusted ratios next to
  the quoted ratios of Tables~\ref{tab:headline} and~\ref{tab:hedged}.
  The corrected annualization is
  \[
    SR_{\mathrm{ann}} \;=\; SR_{\mathrm{m}}
      \cdot \frac{12}{\sqrt{12 + 2\sum_{k=1}^{11} (12-k)\,\rho_k}},
  \]
  where $SR_{\mathrm{m}}$ is the monthly Sharpe ratio and $\rho_k$ the
  lag-$k$ autocorrelation of monthly returns. Positive autocorrelation
  lowers the annualized ratio, and $\rho_k = 0$ recovers $\sqrt{12}$.
  Long-only monthly returns are positively autocorrelated
  ($\rho_1 \approx 0.19$ full sample). The effect on the IR is within
  $\pm 10\%$ and unstable in sign. The hedged series is more sensitive.
  The persistence enters through the hedge itself: shorting a
  cap-weighted instrument against an equal-weight universe leaves a
  residual spread between the two, and that spread is positively
  autocorrelated. The hedged return decomposes as
  \[
    r^{\mathrm{hedged}}_t \;=\;
    (r^{P}_t - r^{B}_t) \;+\; (r^{B}_t - \beta_t\, r^{X}_t) \;-\; c_t,
  \]
  where $r^{P}$, $r^{B}$, and $r^{X}$ are the portfolio, equal-weight
  benchmark, and hedge-instrument returns, $\beta_t$ the rolling hedge
  ratio, and $c_t$ costs. The first bracket is stock selection. The
  second is that residual spread, and it carries the persistence.
  In realized terms the persistence means that hedged losses cluster:
  drawdowns build over sequences of consecutive small monthly losses
  rather than in single large months. Out-of-sample
  estimates rest on 71 months and are noisy. The significance tests, including the hedged CAPM
  alphas, are HAC-based throughout and unaffected. Together with the zero
  risk-free convention, ratio levels should be compared within this
  document rather than across publications.
  \item \textbf{Sampling error of the ratios.} The ratio point estimates
  carry material estimation error. Estimated over $T$ months, the
  monthly Sharpe ratio $SR_{\mathrm{m}}$ has asymptotic standard error
  \[
    \mathrm{SE}(SR_{\mathrm{m}}) \;=\;
      \sqrt{\frac{1 + SR_{\mathrm{m}}^{2}/2}{T}}
  \]
  \citep{lo2002}; estimate and standard error annualize with the same
  $\sqrt{12}$. This gives approximately 0.4 over the 71 out-of-sample
  months and 0.25 over the full sample. Table~\ref{tab:sharpe_uncertainty}
  reports the resulting 95\% confidence intervals. The MP out-of-sample
  Sharpe is within two standard errors of zero. The full-sample rows
  pool both windows (227 months) and calibrate the wide out-of-sample
  intervals: under a Sharpe that is stable across the sample, both zero
  and the upper ends of the out-of-sample intervals are implausible.
  The excess-return $t$-statistics test the mean directly with
  HAC standard errors and already embed their own sampling error.
  ...
  $-6.8\%$ and $-5.9\%$ excess in a month whose absolute returns were
  positive. In sample,
  ...
  \begin{itemize}
    ...
    on.
    \item \emph{Observable volatility state.} Volatility-managed scaling
    of the hedged spread \citep{barroso2015} was tested with estimation
    windows from six months of monthly returns to the 126-day daily
    estimator of the original rule, conditioning on the spread's own
    volatility and on an in-universe momentum factor's volatility: no
    configuration beats constant sizing at matched volatility by more
    than 0.06 Sharpe in any sample cell. The crash risk such rules
    de-lever sits in momentum's conditional market beta, which turns
    sharply negative after market declines \citep{daniel2016} and is
    concentrated in a short loser leg this book does not carry, while
    the hedge holds the remaining market exposure near zero.
...
\begin{minipage}{\textwidth}
  ...
  \toprule
   &  &  & \multicolumn{2}{c}{Annualized Sharpe} & \multicolumn{2}{c}{Sampling error} \\
  \cmidrule(lr){4-5}\cmidrule(lr){6-7}
  Series & Window & $N$ & $\sqrt{12}$ & Adjusted & SE & 95\% CI \\
  \midrule
  MP long-only & full & 227 & 0.84 & 0.63 & 0.23 & $[0.38, 1.30]$ \\
  MP long-only & OOS  &  71 & 0.71 & 0.62 & 0.42 & $[-0.11, 1.52]$ \\
  MI long-only & full & 227 & 1.06 & 0.77 & 0.24 & $[0.60, 1.52]$ \\
  MI long-only & OOS  &  71 & 0.99 & 0.81 & 0.42 & $[0.17, 1.82]$ \\
  MP hedged & OOS & 71 & 0.85--1.13 & 0.73--0.98 & 0.42 & $[0.04, 1.96]$ \\
  MI hedged & OOS & 71 & 1.44--1.68 & 0.90--1.05 & 0.43 & $[0.60, 2.53]$ \\
  \bottomrule
  \end{tabular}
  \captionof{table}{Annualized Sharpe ratios with autocorrelation adjustment and
  sampling error, all net of modeled costs (long-only rows on the basis
  of Table~\ref{tab:headline}, hedged rows on the net column of
  Table~\ref{tab:hedged}). The two point-estimate columns annualize the
  same monthly Sharpe ratio with $\sqrt{12}$ and with the
  autocorrelation-corrected factor. SE and the 95\% confidence interval
  are the asymptotic values for the $\sqrt{12}$-annualized estimate.
  The interval does not apply to the Adjusted column, whose sampling
  error is larger and is not quoted. Hedged ranges
  span the $[0\%, +2\%]$ dividend adjustment and the interval spans the
  bracket. Hedged rows cover the out-of-sample window only: the rolling
  24-month beta warmup removes 2008--09 from the hedged sample, so a
  full-period hedged row would not be comparable to the long-only full
  period.}
  \label{tab:sharpe_uncertainty}
  \end{minipage}
\vspace{\baselineskip}
...
factors $\to$ backtest, with post-hoc diagnostics separated from the core
pipeline. Correctness rests on three mechanisms. First, invariant
