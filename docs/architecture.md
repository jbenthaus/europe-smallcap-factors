...
This document describes the structure and design of the research pipeline
behind the [research note](../paper/europe_smallcap_factors.pdf). The source
is private; the code excerpts are verbatim, lightly trimmed.
...
flowchart LR
  ...
  subgraph PROD["core pipeline (run_pipeline.py)"]
    direction LR
...
A single entry point runs the core pipeline. Diagnostics are separate
...
common/
  ...
  strategy_specs.py        the factor and strategy registry
...
diagnostics/               offline analyses of backtest output (net-of-cost metrics,
                           alpha ladder, hedge overlay, tail diagnostics, country
                           attribution), with vintage-pinned external data caches
                           committed alongside
...
computed by explicit month-end lookups keyed by calendar arithmetic, so a
missing target month yields a missing forward. When a stock
leaves the panel for some months and returns, positional operations silently
align a formation month with a return realized several months later,
corrupting momentum windows and leaking the wrong month into the dependent
variable.
...
public. Ad-hoc analyses that join filings to months by other routes re-check
the same invariant with a runtime assertion that no row uses a filing that
became available after the formation month.
...
**3. A single registry for factor semantics.** Whether a factor is
...
A test asserts that no module defines a local copy. The entries below show
a market-data factor with no lag and an annual accounting factor with a
90-day publication lag and a staleness cap:
...
**4. Selection does not condition on outcomes.** Forward returns are the
dependent variable only. Portfolio membership is a function of factor ranks
alone; months with missing forward returns contribute fewer names to that
month's realized portfolio return. A test verifies that setting all forward
returns to missing leaves the selected portfolios unchanged.
...
**6. Reproducibility.** Input caches are
...
**7. Core pipeline and exploration are separated.** The core modules are
small and stable, and changes to them go through a per-change review
workflow. Exploratory work lives in a separate investigation area, and
findings are recorded in a chronological research log.
...
- **Property-based suites** (Hypothesis): hundreds of randomized panels per
  run, varying entities, gap patterns, and lengths, asserting invariants
  such as locality (a forward depends only on its own two months) and
...
One property test asserts that an entity's forward returns are identical
whether it is processed alone or alongside other entities, so there is no
cross-contamination between stocks in the timing primitive. Hypothesis
searches for a counterexample panel.
